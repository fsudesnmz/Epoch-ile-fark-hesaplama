...
//Tarih ve saat bilgilerini tutacak struct yapýsý.
struct DateTime {
	struct {
	 int year;
     int month;
...
// DateTime struct'ýný epoch'a çeviren fonksiyon
...
//Bu sistem, bilgisayar sistemlerinde tarih ve saat iþlemlerini kolaylaþtýrýr.
time_t convertToEpoch(struct DateTime dt) {
    struct tm timeinfo = {0};
    timeinfo.tm_year = dt.date.year - 1900; // tm_year 1900'den baþlar
    timeinfo.tm_mon = dt.date.month - 1;    // tm_mon 0'dan baþlar
...
//Kullanýcýdan birinci ve ikinci tarihler alýnýr.	
...
//Epoch deðerleri ekrana yazýlýr.
    printf("Birinci tarihin epoch\'u: %ld\n",(long)epoch1);
    printf("Ikinci tarihin epoch\'u: %ld\n",(long)epoch2);
    
//Tarihler arasýndaki farký hesaplama ve ekrana gösterme.
